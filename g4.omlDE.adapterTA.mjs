...
// ============================================================================
// 範例: 用 TA 當「控制器」自適應 DE 的超參數
//
// 架構:
//   omlDE (被控制) — 透過 funGenerationBefore / funGenerationAfter 兩接口跟外部對話
//   _adapterTA (控制器) — caller 提供 schema 列出要 adapt 的 DE 超參數
//
// TA 跟 SA 對比:
...
//
// 題目: 沿用 g4 同款 3 變數曲線擬合 (對比 omlDE 預設參數跑出之 fitness ≈ 1599)
// ============================================================================
...
async function test() {
    ...
    //TA 要 adapt 的 5 個 DE 參數 schema (格式同 dps: { name, values })
    let schema = [
    ...
    //本題 fitness ≈ 1500-2000, 初始門檻設較大 (threshold=50) 讓初期接受惡化以充分探索
    ...
    //TA 控制 DE: 透過 funGenerationBefore / funGenerationAfter 兩接口接通
