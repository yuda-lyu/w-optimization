...
// ============================================================================
// Benchmark: 比較「無 adapter (omlDE 預設)」與「4 種 adapter」在 g4 題目之效果
// 每設定跑 N 次取平均, 公平比較收斂效率(fitness, stopNg, stopExecutions)
// ============================================================================
...
async function fun(params) {
    let a = params[0]; let b = params[1]; let c = params[2]
    let fitness = 0
    _.each(ps, (v) => {
...
//6 種設定 (統一 Np=20 / NContiguous=100 公平比較)
let baseOpt = { Np: 20, NContiguous: 100 }

let scenarios = [
    {
        name: 'omlDE 預設 + nelderMead LS',
...
async function test() {
    ...
    for (let s of scenarios) {
        ...
        let tStart = Date.now()
        for (let i = 0; i < N; i++) {
            let r = await omlDE(dps, fun, s.build())
            runs.push({
        ...
        console.log(`   stopNg avg:     ${result.stopNgAvg.toFixed(0)}`)
    ...
    //對比表
    console.log('=================================================================')
    console.log('Scenario                            | fitAvg  | stopNg | stopExec ')
    console.log('=================================================================')
    allResults.forEach((r) => {
        console.log(`${r.name.padEnd(36)}| ${r.fitnessAvg.toFixed(2).padStart(7)} | ${String(r.stopNgAvg.toFixed(0)).padStart(6)} | ${String(r.stopExecutionsAvg.toFixed(0)).padStart(8)}`)
    })
...
//node g4.omlDE.benchmarkAdapter.mjs
