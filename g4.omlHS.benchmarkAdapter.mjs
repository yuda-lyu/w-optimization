...
// ============================================================================
// Benchmark: 比較「無 adapter (omlHS 預設)」與「4 種 adapter」在 g4 題目之效果
// 每設定跑 N 次取平均
// ============================================================================
...
let schema = [
    { name: 'hsHMCR', values: w.rang(0, 1, 20) },
...
async function test() {
    ...
    for (let s of scenarios) {
        ...
        let tStart = Date.now()
        for (let i = 0; i < N; i++) {
            let r = await omlHS(dps, fun, s.build())
            runs.push({
        ...
        console.log(`   stopNi avg:     ${result.stopNiAvg.toFixed(0)}`)
    ...
    console.log('=================================================================')
    console.log('Scenario                            | fitAvg  | stopNi | stopExec ')
    console.log('=================================================================')
    allResults.forEach((r) => {
        console.log(`${r.name.padEnd(36)}| ${r.fitnessAvg.toFixed(2).padStart(7)} | ${String(r.stopNiAvg.toFixed(0)).padStart(6)} | ${String(r.stopExecutionsAvg.toFixed(0)).padStart(8)}`)
    })
...
//node g4.omlHS.benchmarkAdapter.mjs
