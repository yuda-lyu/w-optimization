...
// ============================================================================
// Benchmark: 比較「無 adapter (omlRGA 預設)」與「4 種 adapter」在 g4 題目之效果
// 每設定跑 N 次取平均
// ============================================================================
...
let schema = [
    { name: 'rgaMutationRate', values: w.rang(0, 1, 20) },
...
async function test() {
    ...
    for (let s of scenarios) {
        ...
        let tStart = Date.now()
        for (let i = 0; i < N; i++) {
            let r = await omlRGA(dps, fun, s.build())
            runs.push({
...
//node g4.omlRGA.benchmarkAdapter.mjs
