...
// ============================================================================
// 範例: 用 ACO 當「控制器」自適應 PSO 的超參數
//
// 架構:
//   omlPSO (被控制) — 透過 funGenerationBefore / funGenerationAfter 兩接口跟外部對話
//   _adapterACO (控制器) — caller 提供 schema 列出要 adapt 的 PSO 超參數
//
// 每代 PSO 主迴圈:
//   1. funGenerationBefore: 從 ACO suggest() 取一組 params 覆寫本代要用的 PSO 參數
//   2. PSO 跑一代 (更新所有粒子的位置/速度/慣性)
//   3. funGenerationAfter: 把本代 fitness 餵回 ACO feedback() 強化 pheromone
//   4. ACO 經過多代學習, 收斂到「對本問題表現好」的超參數組合
//
// 題目: 沿用 g4 之 3 變數曲線擬合 (對比 omlPSO 預設參數跑出之 fitness ≈ 1599)
// ============================================================================
...
async function test() {
    ...
    //ACO 要 adapt 的 5 個 PSO 參數 schema (格式同 dps: { name, values })
    let schema = [
        { name: 'psoC1', values: w.rang(0, 4, 20) }, //21 階, step 0.2
    ...
    //ACO 控制 PSO: 透過 funGenerationBefore / funGenerationAfter 兩接口接通
